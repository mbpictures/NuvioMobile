...
internal actual object MemberAssetStorage {
    ...
    private val backgroundDirectory: File by lazy { cacheRoot.resolve("profile_backgrounds") }
    private val avatarDirectory: File by lazy { cacheRoot.resolve("profile_avatars") }

    actual fun loadAccessPayload(): String? =
        DesktopPreferences.getString(preferencesName, accessPayloadKey)

    actual fun saveAccessPayload(payload: String) {
        DesktopPreferences.putString(preferencesName, accessPayloadKey, payload)
    }

    actual fun loadProfileBackgroundCatalogPayload(): String? =
        DesktopPreferences.getString(preferencesName, backgroundCatalogPayloadKey)

    actual fun saveProfileBackgroundCatalogPayload(payload: String) {
        DesktopPreferences.putString(preferencesName, backgroundCatalogPayloadKey, payload)
    }
    ...
    actual fun clearAccess() {
        DesktopPreferences.remove(preferencesName, accessPayloadKey)
    }
